...
if (isset($_POST['submit'])) {
    ...
    {
        if($id){
        $sql = "UPDATE menus SET name='$name', categories_id='$categories', note='$note', note='$note',
                price='$price', status='$status' WHERE id=$id";
    }else {
    $sql = "INSERT INTO menus(name, categories_id, note, price, status) VALUES ('$name', '$categories', 
            '$note', '$price', '$status')";
